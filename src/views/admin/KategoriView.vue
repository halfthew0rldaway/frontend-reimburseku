...
const form = ref({ id: null, category_name: '', description: '' })
...
async function loadCategories() {
  loading.value = true
  try {
    const res = await axios.get('http://127.0.0.1:8000/api/category', { headers: getHeaders() })
...
function openAdd() {
  ...
  form.value = { id: null, category_name: '', description: '' }
  showModal.value = true
}

function openEdit(cat) {
  isEditing.value = true
  form.value = { id: cat.id, category_name: cat.category_name, description: cat.description ?? '' }
  showModal.value = true
}
...
async function save() {
  ...
  try {
    ...
    if (isEditing.value) {
      await axios.post(`http://127.0.0.1:8000/api/category/${form.value.id}`, fd, { headers: getHeaders() })
    } else {
      await axios.post('http://127.0.0.1:8000/api/category/save', fd, { headers: getHeaders() })
    }
...
async function deleteCategory() {
  try {
    await axios.delete(`http://127.0.0.1:8000/api/category/delete/${deleteConfirmId.value}`, { headers: getHeaders() })
...
<template>
  <div class="page">
    <div class="page-head">
      <div>
        <h1 class="page-title">Kategori Reimbursement</h1>
        <p class="page-sub">Kelola kategori yang tersedia untuk pengajuan reimbursement</p>
      </div>
      ...
      <table v-else class="data-table">
        ...
        <tbody>
          <tr v-if="categories.length === 0">
            <td colspan="4" class="empty-cell">Belum ada kategori.</td>
          </tr>
          <tr v-for="(cat, i) in categories" :key="cat.id">
            <td class="text-muted">{{ i + 1 }}</td>
            <td class="font-semibold">{{ cat.category_name }}</td>
            <td class="text-muted">{{ cat.description ?? '—' }}</td>
            <td>
              <div class="action-btns">
                <button class="icon-btn edit-btn" @click="openEdit(cat)"><Pencil :size="14" /></button>
                <button class="icon-btn delete-btn" @click="deleteConfirmId = cat.id"><Trash2 :size="14" /></button>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <div v-if="deleteConfirmId" class="modal-overlay" @click.self="deleteConfirmId = null">
      <div class="modal modal-sm">
        ...
        <p class="confirm-text">Kategori ini akan dihapus secara permanen.</p>
        <div class="modal-footer">
